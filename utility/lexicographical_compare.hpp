...
/*   Updated: 2022/03/18 08:45:22 by akhalidy         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
namespace ft
{
		...
		InputIterator2 first2, InputIterator2 last2, Compare comp)
	{
		while (first1 != last1)
		{
			if (first2 == last2 || comp(*first2, *first1)) // si j arrive a la fin du dexieme range, alors A > B ou bien je trouve un elt de b < à elt de a.
				return false;
			else if (comp(*first1, *first2)) // si je trouve un elt de A < B, donc a < b.
				return true;
			++first1; ++first2; // tant que first1 == first2, je passe au elt suivant
		}
		return (first2 != last2); // if first2 != last2 that means that range B is greater than range A  => true
