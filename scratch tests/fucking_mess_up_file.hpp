...
namespace ft
{
	...
	{
		public:
			typedef iterator_traits< iterator<std::random_access_iterator_tag, _Iter> > iterator_traits;
			typedef typename iterator_traits::difference_type   		difference_type;		//* ptrdiff_t		
			typedef typename iterator_traits::value_type        		value_type;				//* T
			typedef typename iterator_traits::pointer           		pointer;				//* T*
			typedef typename iterator_traits::reference         		reference;				//* T&
			typedef typename iterator_traits::iterator_category 		iterator_category;		//* T1 - T2
		...
		random_access_iterator(const random_access_iterator &it) : __current(it.__current) {}
		...
		reference						operator[](difference_type n) const {return (__current[n]);}
		random_access_iterator			operator+(difference_type n) const {return random_access_iterator(__current + n);}//?{random_access_iterator tmp; tmp.__current = __current + n; return tmp;}
		random_access_iterator			operator-(difference_type n) const {return random_access_iterator(__current - n);}//?{random_access_iterator tmp; tmp.__current = __current - n; return tmp;}
		random_access_iterator&			operator+=(difference_type n) {__current += n; return *this;}
		random_access_iterator&			operator-=(difference_type n) {__current -= n; return *this;}


		operator random_access_iterator<const value_type> () const { return random_access_iterator<const value_type>(__current) ; }
		private:
			pointer	__current;
			random_access_iterator(const reference curr) : __current(curr) {}  
	};

	template <typename _IterL> 
	bool operator==(const random_access_iterator<_IterL>& lhs, const random_access_iterator<_IterL>& rhs){ return (lhs.base() == rhs.base()); }
	template <typename _IterL> 
	bool operator!=(const random_access_iterator<_IterL>& lhs, const random_access_iterator<_IterL>& rhs){ return (lhs.base() != rhs.base()); }
	template <typename _IterL> 
	bool operator<(const random_access_iterator<_IterL>& lhs, const random_access_iterator<_IterL>& rhs){ return (lhs.base() < rhs.base()); }
	...
	bool operator>=(const random_access_iterator<_IterL>& lhs, const random_access_iterator<_IterL>& rhs){ return (lhs.base() >= rhs.base()); }
	template <typename _Iter> 
	random_access_iterator<_Iter> operator+(ptrdiff_t n, const random_access_iterator<_Iter>& rhs){ return (n + rhs.base()); }
	template <typename _IterL>
	ptrdiff_t operator-(const random_access_iterator<_IterL>& lhs, const random_access_iterator<_IterL> & rhs){return (lhs.base() - rhs.base());}
